...
class SensitiveContentChecker : InitializingBean {
    private val nodes: MutableMap<Int, WordNode>? = null // 存储敏感词节点
    private val stopWordSet: MutableSet<Int>? = null // 停顿词
    private var sensitiveWordsProvider: SensitiveWordsProvider? = null
    ...
    private fun addSensitiveWord(words: List<String?>?) {
        words?.forEach { word ->
            ...
            if (fnode == null) {
                fnode = WordNode(fchar, charArray.size == 1)
                nodes[fchar] = fnode
            } else {
