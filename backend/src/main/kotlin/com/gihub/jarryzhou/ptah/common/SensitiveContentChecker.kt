...
class SensitiveContentChecker : InitializingBean {
    private val nodes: MutableMap<Int, _root_ide_package_.com.gihub.jarryzhou.ptah.common.WordNode>? = null // 存储敏感词节点
    private val stopWordSet: MutableSet<Int>? = null // 停顿词
    private var sensitiveWordsProvider: _root_ide_package_.com.gihub.jarryzhou.ptah.common.SensitiveWordsProvider? = null
    private var stopWordsProvider: _root_ide_package_.com.gihub.jarryzhou.ptah.common.SensitiveWordsProvider? = null
    var isEnabled = true
    ...
    private fun addSensitiveWord(words: List<String?>?) {
        words?.forEach { word ->
            ...
            if (fnode == null) {
                fnode = _root_ide_package_.com.gihub.jarryzhou.ptah.common.WordNode(fchar, charArray.size == 1)
                nodes[fchar] = fnode
            } else {
    ...
    fun checkText(textContent: String?): String? {
        ...
        var k: Int
        var node: _root_ide_package_.com.gihub.jarryzhou.ptah.common.WordNode?
        // 遍历字符
        var i = 0
        while (i < length) {
            // 获得当前检查字符
